  ...
  <v-app>
    ...
    <v-main class="main-content">
      <div>
        ...
        <p v-for="n in 100" :key="n">Contenu pour tester le scroll n°{{ n }}</p>
      </div>
    </v-main>

    <!-- Footer fixe en bas de l'écran -->
    <v-footer class="fixed-footer" height="60">
      <a
        v-for="item in items"
        :key="item.title"
        :href="item.href"
...
/* Assurez-vous que la page prend toute la hauteur */
.main-content
  min-height: calc(100vh - 60px) // Hauteur du footer déduite de la hauteur totale
  padding: 16px

/* Le footer est fixe en bas de la fenêtre */
.fixed-footer
  position: fixed
  bottom: 0
  left: 0
